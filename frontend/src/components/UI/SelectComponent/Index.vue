  ...
  >
    <div class="custom-select__view">
      <div class="custom-select__view-selected">
        <img :src="getItem(modelValue) ? getItem(modelValue).itemData.img : null" alt="">
        <span>{{getItem(modelValue) ? getItem(modelValue).itemData.symbol : null}}</span>
      </div>
      <div class="custom-select__view-search">
        <input class="input" type="text" v-model.trim="searchItem" ref="tokenInputElement" :placeholder="'search'">
      </div>
    ...
    <div class="custom-select__options">
      <template v-if="viewOptionsList && viewOptionsList.length">
        <div
          class="option"
          v-for="optionItem in viewOptionsList"
          :key="optionItem.id"
          @click="selectToken(optionItem.itemData)"
        >
          <img :src="getItem(optionItem.itemData.id) ? getItem(optionItem.itemData.id).itemData.img : null" alt="">
          <span>{{getItem(optionItem.itemData.id) ? getItem(optionItem.itemData.id).itemData.symbol : null}}</span>
        </div>
      </template>
...
export default {
  ...
  computed: {
    getItem() {
      return (itemId) => this.filteredList.find((data) => data.itemData.id === itemId)
    },

    filteredList() {
      console.log(this.listOptions, this.listKey, 'tokensPolygon')
      let activeTokens = []

      if (this.listKey) {
        activeTokens = this.listOptions[this.listKey]
      }

      if (this.listKey === '') {
        activeTokens = this.listOptions
      }
      console.log(activeTokens, 'activeTokens')
      
      return activeTokens.map((data, index) => ({ itemData: data, id: index }))
    },
    ...
    viewOptionsList() {
      ...
      //     return data.itemData.id.toLowerCase().token.includes(query)
      //   })
      // }
...
.custom-select{
  ...
  &.custom-select_mode-choose-open {
